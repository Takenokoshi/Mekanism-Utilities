...
public class MUEitherSideInputRecipeCache<INPUT, INGREDIENT extends InputIngredient<INPUT>, RECIPE extends MekanismRecipe<?> & BiPredicate<INPUT, INPUT>, CACHE extends IInputCache<INPUT, INGREDIENT, RECIPE>>
    ...
    public boolean containsInput(@Nullable Level world, INPUT inputA, INPUT inputB) {
        if (cache.isEmpty(inputA)) {
            // Note: We don't bother checking if b is empty here as it will be verified in
            // containsInputB
        ...
        initCacheIfNeeded(world);
        // Note: Even though we know the cache contains input A, we need to check both
        // input A and input B
        // This is because we want to ensure that we allow the inputs being in either
        // order, but in our
        // secondary validation we check inputB first as we know the recipe contains
        // inputA as one of the
        // inputs, but we want to make sure that we only mark it as valid if the same
        // input is on both sides
        // if the recipe combines two of the same type of ingredient
        if (containsInput(inputA, inputB, cache.getRecipes(inputA))) {
            return true;
        }
        // Our quick lookup cache does not contain it, check any recipes where the
        // ingredients are complex
    ...
    private boolean containsInput(INPUT inputA, INPUT inputB, Iterable<RECIPE> recipes) {
        for (RECIPE recipe : recipes) {
            ...
            if (ingredientB.testType(inputB) && ingredientA.testType(inputA)
                    || ingredientA.testType(inputB) && ingredientB.testType(inputA)) {
                return true;
            }
        }
        return false;
